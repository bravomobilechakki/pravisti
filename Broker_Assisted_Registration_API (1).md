Broker_Assisted_Registration_API
...
Description
Retrieves all Broker Assisted Onboarding records created by the logged-in Broker, including invited user details, company information, linked deals, and current onboarding status.
Authentication
Required: Bearer Token (Broker)
Request
GET /api/broker-onboard/my-deal
...
role
String
Counterparty role (buyer or seller).
status
String
Current onboarding status (e.g. pending, approved).
invitedMobile
String
Mobile number used for onboarding.
targetUserName
String
Name of the invited user.
company
Object
Company details of the invited user.
deals
Array
List of deals associated with the registration.
createdAt
String
Registration creation timestamp.

Business Logic
Returns all onboarding records created by the logged-in Broker.
Includes company details and associated deals for each registration.
Helps the Broker track onboarding progress and linked deals.
Supports both Buyer and Seller onboarding records.
...
productStatus: pending
The seller has not verified the products yet.
Business Logic
...
POST http://localhost:8081/api/broker-onboard/verify-account
Description
This API is the first step of the Broker Assisted Onboarding process. It is used by the invited Trader to confirm ownership of the account created by the Broker. Once the Trader approves the ownership, the system activates the account and updates the onboarding progress. Based on the Trader's role (Buyer or Seller), the system automatically determines the next onboarding step or marks the onboarding as completed.
Authentication
Required: Bearer Token (Invited Trader)
Authorization: Bearer <TRADER_ACCESS_TOKEN>
Request Body
...
String
Yes
Ownership verification decision. Supported value: approved.
...

{
  "statusCode": 200,
  "data": {
    "message": "Ownership confirmed and account activated successfully",
    "completed": true
  },
...
data.message
String
Confirms that the account ownership has been verified and activated successfully.
data.completed
Boolean
Indicates whether the onboarding process is completed after account verification.

Business Logic
This API is called after the invited Trader logs into the application.
Verifies that the logged-in Trader is the actual owner of the account created by the Broker.
On successful verification, the Trader's account is activated.
Updates the onboarding status for the current registration.
Based on the Trader's role and onboarding configuration, the backend decides whether additional onboarding steps are required.
If all required onboarding steps are completed, the API returns completed: true.
If additional steps are required (such as Company or Product Verification), the frontend should continue with the next onboarding screen.
After successful verification, the Trader is allowed to continue the onboarding process or access the dashboard based on the value completed.
This API can only be used for pending onboarding registrations and by the invited Trader associated with that registration.
...
How This API Works
The Seller selects a pending Product Access Request.
The backend checks that the logged-in Seller is authorized to manage the Seller company linked to the request.
It verifies that the request currently has a pending status.
The Seller submits either approved or rejected.
When approved, the request status changes from pending to approved.
The backend stores the Seller user ID in resolvedBy.
The approval date and time are stored in resolvedAt.
The related Draft Deal remains in draft status until the Broker completes it.
After approval, the Broker can add quantities, prices, discounts, and submit the final deal.
When rejected, the Broker should not be allowed to complete the draft deal using those products.
Current Testing Result
...
1
POST /api/deals/broker/draft
Broker ek Draft Deal create karta hai aur Seller ko Product Access Request bhejta hai taaki seller apne products broker ke saath share kar sake.
Broker
2
GET /api/deals/broker/product-access?companyId=<SELLER_COMPANY_ID>
Seller apni company ke liye aayi hui Pending Product Access Requests ki list fetch karta hai.
Seller
3
PATCH /api/deals/broker/product-access/:requestId
Seller request ko Approve ya Reject karta hai. Approve hone par broker seller ke products dekh sakta hai.
Seller
4
PUT /api/deals/broker/draft/:dealId/complete
Product access approve hone ke baad Broker deal ki remaining details jaise Products, Quantity, Price, GST, Discount, Payment Terms fill karke Draft Deal ko complete karta hai aur final deal create karta hai.
Broker
